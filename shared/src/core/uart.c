...
#define USART2_DATA_BITS (8)
#define USART2_BAUDRATE (115200)
#define RING_BUFF_SIZE (128) // For maximum of ~10ms of throughput

static ring_buffer_t rb = {0U};

static uint8_t data_buffer[RING_BUFF_SIZE] = {0U};
...
{
    ring_buffer_setup(&rb, data_buffer, RING_BUFF_SIZE);

    rcc_periph_clock_enable(RCC_USART2);

    usart_set_mode(USART2, USART_MODE_TX_RX);
    usart_set_flow_control(USART2, USART_FLOWCONTROL_NONE);
    usart_set_databits(USART2, USART2_DATA_BITS);
